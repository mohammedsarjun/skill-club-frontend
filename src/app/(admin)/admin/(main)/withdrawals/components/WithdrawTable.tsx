...
// component expects props for optional filters
type Props = {
...
const WithdrawalDetailsModal = ({
  ...
  const statusColors: Record<string, string> = {
    ...
    "withdrawal_requested": "bg-blue-100 text-blue-800",
      ...
      <div className="bg-white rounded-2xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-8 space-y-6">
          ...
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-6 border border-purple-100">
            ...
            <div className="flex items-start gap-4 mb-4">
              <img 
                ...
                onError={(e) => {
                  e.currentTarget.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(withdrawal.freelancer.name)}&background=random`;
                }}
              />
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <h4 className="text-xl font-semibold text-gray-900">{withdrawal.freelancer.name}</h4>
                  {withdrawal.freelancer.isVerified && (
                    <svg className="w-5 h-5 text-blue-500" fill="currentColor" viewBox="0 0 20 20">
                      <path fillRule="evenodd" d="M6.267 3.455a3.066 3.066 0 001.745-.723 3.066 3.066 0 013.976 0 3.066 3.066 0 001.745.723 3.066 3.066 0 012.812 2.812c.051.643.304 1.254.723 1.745a3.066 3.066 0 010 3.976 3.066 3.066 0 00-.723 1.745 3.066 3.066 0 01-2.812 2.812 3.066 3.066 0 00-1.745.723 3.066 3.066 0 01-3.976 0 3.066 3.066 0 00-1.745-.723 3.066 3.066 0 01-2.812-2.812 3.066 3.066 0 00-.723-1.745 3.066 3.066 0 010-3.976 3.066 3.066 0 00.723-1.745 3.066 3.066 0 012.812-2.812zm7.44 5.252a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                    </svg>
                  )}
                </div>
                <p className="text-sm text-gray-600">{withdrawal.freelancer.profile.professionalRole}</p>
                <p className="text-sm text-purple-600 font-medium">{withdrawal.freelancer.profile.workCategory}</p>
              </div>
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <p className="text-sm text-gray-600">Email</p>
                <p className="text-sm font-medium text-gray-900">{withdrawal.freelancer.email}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Phone</p>
                <p className="text-sm font-medium text-gray-900">{withdrawal.freelancer.phone}</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Hourly Rate</p>
                <p className="text-sm font-medium text-gray-900">₹{withdrawal.freelancer.profile.hourlyRate}/hr</p>
              </div>
              <div>
                <p className="text-sm text-gray-600">Account Status</p>
                <div className="flex gap-2 mt-1">
                  {withdrawal.freelancer.isBlocked ? (
          ...
          <div className="flex gap-3 pt-4">
            ...
            {withdrawal.transaction.status === "pending" || withdrawal.transaction.status === "withdrawal_requested" ? (
              <>
...
// ===== Main Component =====
export default function WithdrawalListingPage({ roleFilter, statusFilter }: Props) {
  ...
  // Transform data for table display (minimal columns)
  const tableData = withdrawals.map((w) => ({
    id: w.id || w.transaction.transactionId,
    transactionId: w.transaction.transactionId,
    name: w.freelancer.name,
  ...
  const handleView = (row: { id: string }) => {
    const fullRecord = withdrawals.find((w) => (w.id || w.transaction.transactionId) === row.id);
    if (fullRecord) setSelectedWithdrawal(fullRecord);
  };
  ...
  return (
    <>
      ...
      <WithdrawalDetailsModal withdrawal={selectedWithdrawal} onClose={() => setSelectedWithdrawal(null)} />
