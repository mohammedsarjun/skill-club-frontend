...
interface AdminSplitFundsModalProps {
  isOpen: boolean;
  onClose: () => void;
  heldAmount: number;
  onSubmit: (clientPercentage: number, freelancerPercentage: number) => Promise<void>;
}

export const AdminSplitFundsModal = ({ 
  isOpen, 
  onClose, 
  heldAmount, 
  onSubmit 
}: AdminSplitFundsModalProps) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSubmit(clientPercentage, freelancerPercentage);
    } finally {
    ...
    <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
      <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
        <div className="flex items-center justify-between p-6 border-b">
          <h2 className="text-xl font-semibold text-gray-900">Split Dispute Funds</h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4 mt-6">
              <div className="bg-blue-50 rounded-lg p-4 border-2 border-blue-200">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-gray-700">Client Refund</span>
                </div>
              ...
              <div className="bg-green-50 rounded-lg p-4 border-2 border-green-200">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-medium text-gray-700">Freelancer Release</span>
                </div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
            <p className="text-sm text-yellow-800">
              This action will resolve the dispute and split the held milestone funds according to the percentages set above.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3 p-6 border-t bg-gray-50">
          ...
          >
            {isSubmitting ? 'Processing...' : 'Split Funds'}
          </button>
        </div>
      </div>
