...
interface ClientRejectModalProps {
  open: boolean;
  meetingId?: string;
  onClose: () => void;
...
export default function ClientRejectModal({ open, meetingId, onClose, onSubmit }: ClientRejectModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg w-full max-w-md p-6">
        <h3 className="text-lg font-semibold mb-3">Reject Meeting</h3>
        <textarea
          value={reason}
          onChange={(e) => setReason(e.target.value)}
          className="w-full border border-gray-200 rounded-md p-2 mb-4 h-28"
          placeholder="Provide reason for rejecting the meeting"
        />
