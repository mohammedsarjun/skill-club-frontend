...
interface TodayMeetingsProps {
  ...
  ongoingMeetings: IFreelancerMeetingListItem[];
...
const TodayMeetings: React.FC<TodayMeetingsProps> = ({
  ...
  const getOtherParty = (meeting: IFreelancerMeetingListItem) => {
    return currentUserRole === 'freelancer' ? meeting.client : null;
  };
        ...
        <div className="mb-6">
          ...
          {ongoingMeetings.map((meeting) => {
            ...
            const displayName = otherParty
              ? otherParty.companyName || `${otherParty.firstName || ''} ${otherParty.lastName || ''}`.trim()
              : 'Unknown';
            const avatarUrl = otherParty?.logo || `https://ui-avatars.com/api/?name=${encodeURIComponent(displayName)}&background=3b82f6&color=fff`;
            ...
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="text-left py-3 px-4 text-sm font-semibold text-gray-700">
                  {currentUserRole === 'freelancer' ? 'Client' : 'Freelancer'}
                </th>
