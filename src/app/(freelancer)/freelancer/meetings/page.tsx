...
import { startAgora } from "@/utils/agoraInitialize";
...
const MeetingsPage = () => {
  ...
  const sentRequests = useMemo(() => {
    return meetings.filter(
      (m) => m.isProposedByFreelancer === true && m.status === "proposed",
    );
  }, [meetings]);

  const receivedRequests = useMemo(() => {
    return meetings.filter(
      (m) => m.isProposedByFreelancer === false && m.status === "proposed",
    );
  }, [meetings]);
  ...
  const handleRejectMeeting = useCallback(
    async (meetingId: string) => {
      try {
        const response = await freelancerActionApi.rejectMeeting({
          meetingId,
  ...
  const handleApproveReschedule = useCallback(
    async (meetingId: string) => {
      try {
        const response = await freelancerActionApi.approveReschedule({
          meetingId,
        });
  ...
  const handleDeclineReschedule = useCallback(
    async (meetingId: string) => {
      try {
        const response = await freelancerActionApi.declineReschedule({
          meetingId,
    ...
    <div className="max-w-7xl mx-auto p-6 bg-gray-50 min-h-screen">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Manage your client meetings and schedule
        </p>
      </div>
