...
const MeetingsPage = () => {
  ...
  const fetchRescheduledMeetings = useCallback(async (page = 1) => {
    try {
      ...
      if (response?.success && response?.data) {
        ...
        toast.error(response?.message || 'Failed to fetch rescheduled meetings');
      }
    } catch (error) {
  ...
  }, [fetchMeetings, fetchCompletedMeetings, fetchExpiredMeetings, fetchRescheduledMeetings]);
  ...
  const rescheduledRequests = useMemo(() => {
    return rescheduledMeetings.filter((m) => 
      m.rescheduleRequestedBy !== undefined
    );
  }, [rescheduledMeetings]);
  ...
  const getStatusBadge = useCallback((status: string) => {
    ...
    return (
      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${config.color}`}>{config.text}</span>
    );
  }, []);

  const getMeetingTypeBadge = useCallback((meetingType: string) => {
    if (meetingType === 'pre-contract') {
      return <span className="px-3 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-800">Pre-Contract</span>;
    }
    return <span className="px-3 py-1 rounded-full text-xs font-semibold bg-indigo-100 text-indigo-800">Post-Contract</span>;
  }, []);
              ...
              {activeTab === 'rescheduled' && (
                <>
                  {rescheduledLoading ? (
                    <div className="text-center py-12">
                      <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div>
                      <p className="mt-4 text-gray-600">Loading rescheduled meetings...</p>
                    </div>
                  ) : (
                    <>
                      <RescheduledRequests
              ...
              {activeTab === 'completed' && (
                <>
                  {completedLoading ? (
                    <div className="text-center py-12">
                      <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div>
                      <p className="mt-4 text-gray-600">Loading completed meetings...</p>
                    </div>
                  ) : (
                    <>
                      <CompletedMeetings
              ...
              {activeTab === 'expired' && (
                <>
                  {expiredLoading ? (
                    <div className="text-center py-12">
                      <div className="inline-block animate-spin rounded-full h-12 w-12 border-b-2 border-blue-600"></div>
                      <p className="mt-4 text-gray-600">Loading expired meetings...</p>
                    </div>
                  ) : (
                    <>
                      <ExpiredMeetings
