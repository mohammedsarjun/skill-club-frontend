...
import FreelancerDeclineModal from "./FreelancerDeclineModal";
...
const MeetingPanel = ({ contractId }: MeetingPanelProps) => {
  ...
  async function joinMeet(meetingId: string) {
    try {
      const res = await freelancerActionApi.joinMeet(meetingId);
      const data = res.data;
      const { startAgora } = await import("@/utils/agoraInitialize");
      await startAgora(data);
      router.push(`/freelancer/meetings/${meetingId}/video`);
    } catch (error) {
  ...
  const loadMeetings = async () => {
    try {
      setLoading(true);
      const response = await freelancerActionApi.getContractMeetings(
        contractId
      );
  ...
  const handleAccept = async (meetingId: string) => {
    try {
      setLoading(true);
      const res = await freelancerActionApi.acceptMeeting({ meetingId });
      if (res?.success) {
  ...
  const [counterRescheduleModalOpen, setCounterRescheduleModalOpen] = useState(false);
  const [selectedMeetingToCounterReschedule, setSelectedMeetingToCounterReschedule] =
    useState<string | undefined>(undefined);
  ...
  const submitReschedule = async (meetingId: string, proposedTime: string) => {
    try {
      setLoading(true);
      const res = await freelancerActionApi.requestMeetingReschedule({
        meetingId,
        proposedTime,
      });
      if (res?.success) {
  ...
  const submitReject = async (meetingId: string, reason: string) => {
    try {
      setLoading(true);
      const res = await freelancerActionApi.rejectMeeting({
        meetingId,
        reason,
      });
      if (res?.success) {
  ...
  const handleApproveReschedule = async (meetingId: string) => {
    try {
      setLoading(true);
      const res = await freelancerActionApi.approveReschedule({ meetingId });
      if (res?.success) {
  ...
  const submitDecline = async (meetingId: string, reason: string) => {
    try {
      setLoading(true);
      const res = await freelancerActionApi.declineReschedule({ meetingId, reason });
      if (res?.success) {
        await loadMeetings();
      } else {
  ...
  const openCounterRescheduleModal = (meetingId: string) => {
    setSelectedMeetingToCounterReschedule(meetingId);
    setCounterRescheduleModalOpen(true);
  };

  const closeCounterRescheduleModal = () => {
    setSelectedMeetingToCounterReschedule(undefined);
    setCounterRescheduleModalOpen(false);
  };

  const submitCounterReschedule = async (meetingId: string, proposedTime: string) => {
    try {
      setLoading(true);
      const res = await freelancerActionApi.counterReschedule({ meetingId, proposedTime });
      if (res?.success) {
        await loadMeetings();
      } else {
        throw new Error("Counter reschedule failed");
      }
    } finally {
      setLoading(false);
    }
    ...
    switch (type) {
      ...
      case "requests":
        return meetings
          .filter((m) => {
            return (
              (["proposed"].includes(m.status) && !m.isProposedByFreelancer) ||
  ...
  const MeetingCard = ({
    ...
    meeting: IFreelancerMeetingListItem;
  }) => (
    <div className="bg-white border border-gray-200 rounded-lg p-5 hover:shadow-md transition-shadow">
      ...
      {meeting.status === "rescheduled_requested" &&
      ...
      {meeting.status === "proposed" && !meeting.isProposedByFreelancer && (
        <div className="flex gap-2 mt-4">
          <button
            onClick={() => handleAccept(meeting.meetingId)}
          ...
          <button
            onClick={() => handleReschedule(meeting.meetingId)}
      ...
      {meeting.status === "reschedule_requested" && meeting.rescheduleRequestedBy === "client" && (
        <div className="flex gap-2 mt-4">
          <button 
          ...
          <button 
            onClick={() => openCounterRescheduleModal(meeting.meetingId)}
            className="flex-1 bg-blue-600 text-white py-2 px-4 rounded-lg hover:bg-blue-700 transition-colors flex items-center justify-center gap-2"
          >
            <Clock className="w-4 h-4" />
            Reschedule
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      />
      <RescheduleModal
        open={counterRescheduleModalOpen}
        meetingId={selectedMeetingToCounterReschedule}
        onClose={closeCounterRescheduleModal}
        onSubmit={submitCounterReschedule}
      />
    </div>
  );
};
